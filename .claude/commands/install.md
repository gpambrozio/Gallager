...
* Compile, install and start the app on the myiPhone device. Use the `device-app` skill in `XcodeBuildTools` plugin. If the device is not connected let the user know. Only do this in "all" mode or if "phone" is specified in $ARGUMENTS

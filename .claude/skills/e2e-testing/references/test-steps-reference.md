...
Take a screenshot of the iOS simulator. The label is auto-prefixed with a zero-padded counter (`01-`, `02-`, etc.) that resets per scenario — do not add manual number prefixes. By default, the screenshot is compared against a stored baseline (see `docs/e2e-testing.md` for screenshot comparison details). Only use `compare: false` if the screen would change between runs.
...
Take a screenshot of the macOS app window. The label is auto-prefixed with a zero-padded counter (`01-`, `02-`, etc.) that resets per scenario — do not add manual number prefixes. By default, the screenshot is compared against a stored baseline (see `docs/e2e-testing.md` for screenshot comparison details). Only use `compare: false` if the screen would change between runs.

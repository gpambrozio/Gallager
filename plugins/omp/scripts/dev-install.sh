...
# after editing bin/sidecar or omp-bridge/gallager.ts, then relaunch Gallager.
#
#   ./scripts/dev-install.sh            # copy ~/.gallager/plugins/omp  (discoverable)
...
# launch), then `gallager plugin list` shows `omp` (source "folder"). In Settings,
# enable it and click Install to drop the omp bridge extension into
# ~/.omp/agent/extensions/gallager.ts.
set -euo pipefail

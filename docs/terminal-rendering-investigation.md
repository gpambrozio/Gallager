...
#### 1. TCP Read Splitting → Escape Sequence Fragmentation (FOUND AND FIXED)
...
**Fix:** Generation counter + 2ms delay coalescing in `TmuxControlClient`:
```swift
private var dataGeneration: UInt64 = 0

private func processIncomingData(_ data: Data) async {
    dataGeneration &+= 1
    // ... parse lines, accumulate output ...
    scheduleFlush()
}
...
**Result:** Effectively eliminated frame splitting (158/160 complete frames, zero `STARTS_DIGIT` entries). But **artifacts persisted**, proving this was not the sole cause.
...
#### 2. `needsLayout = true` After Every Feed (FOUND AND FIXED, DID NOT RESOLVE ARTIFACTS)
...
- **Feed logging** (`TerminalContainerView.logFeed`): Logs every feed to `/tmp/claudespy-feeds.txt` with size, first/last bytes, truecolor BG count, bare CSI parameter detection, and starts-with-digit detection. Limited to first 500 feeds.
- **Frame capture** (`/tmp/claudespy-frames.bin`): Binary capture of all frames fed to SwiftTerm with 4-byte little-endian length prefix per frame.
- **Frame replay** (`terminal-debug/replay-frames.py`): Python script to replay captured frames in a real terminal for visual comparison.
- **Buffer dump**: Direct SwiftTerm buffer content extraction after feeding, comparing buffer state against expected content.

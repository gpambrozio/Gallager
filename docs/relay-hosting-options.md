...
2. **DNS:** add a second A record (e.g. `relay-new.gustavo.eng.br`) pointing at the new IP for testing. Don't repoint `gallager.gustavo.eng.br` (or its legacy alias `claudespy.gustavo.eng.br`) yet.
...
6. **Cutover:** repoint `gallager.gustavo.eng.br` (and the legacy `claudespy.gustavo.eng.br`) to the new IP. Caddy on the new box re-issues the certs via ACME.

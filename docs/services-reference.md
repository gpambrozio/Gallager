...
- Decoding lives in `OTLPModels.swift` (tolerant: int64 may arrive as a JSON string or number). Accumulation lives in `OTLPTelemetryAccumulator.swift` (pure value logic, unit-tested), keyed by `session.id`:
  ...
  - `claude_code.commit.count` / `pull_request.count` counters → milestone deltas between exports **and** the cumulative count carried onto the snapshot (issue #598, for the recap).
  - `claude_code.active_time.total` counter → cumulative active seconds; `claude_code.lines_of_code.count` (`type=added/removed`) → per-type line counts; `claude_code.tool_result` log events → a per-event tool count (issue #598).

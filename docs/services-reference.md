...
- Decoding lives in `OTLPModels.swift` (tolerant: int64 may arrive as a JSON string or number). Accumulation lives in `OTLPTelemetryAccumulator.swift` (pure value logic, unit-tested), keyed by the session join id (Claude `session.id` / Codex `conversation.id`):
  ...
  - **Codex** `codex.sse_event` (`event.kind = response.completed`) → tokens + `model`. OpenAI's `cached_token_count` is nested *inside* `input_token_count` (unlike Claude's disjoint buckets), so it's mapped to the cache-read field and the headline total stays `input + output` (the cached re-read isn't double-counted). Codex emits no cost.
  - **Codex** `codex.api_request` → the turn's `duration_ms` (`SessionTelemetry.recordTurnLatency`, which back-fills the just-completed token turn's sparkline point). Tokens and latency arrive on *separate* events because only these two log events carry `conversation.id` — Codex metrics omit it (openai/codex#15905), so the `codex.turn.*` metrics can't be joined to a pane and are ignored.

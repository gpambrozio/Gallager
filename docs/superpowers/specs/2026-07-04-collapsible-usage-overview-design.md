...
- **E2E:** `OTELUsageOverviewScenario` keeps its collapsed-state wait
  ...
  state. The changed/new `mac-usage-overview*` baselines are removed locally
  (`git rm`) and regenerated by CI per repo policy.

...
## SPM `.embedInCode` resources break the macOS app build (do not retry)

**Status:** Won't fix here — Xcode limitation. Documented so the "obvious cleanup" isn't attempted again.

Shipping `GallagerEmoji`'s emoji table as a data file with SPM's
`resources: [.embedInCode(...)]` (instead of the generated
`EmojiData.swift` string literal) was designed, implemented, and reverted
on PR #632 (spec: `docs/superpowers/specs/2026-07-03-emoji-data-shipping-design.md`).

It works everywhere **except** the one build that matters: the macOS
`ClaudeSpyServer` app. Verified on Xcode 26.6 via build-log inspection —
when a resource-bearing package target is consumed by **two products in
the same build graph** (the app's library stack *and* the `GallagerCLI`
executable product, which the app target links to force its build for the
copy-into-bundle phase), XCBuild silently ignores `.embedInCode`:

- generates the legacy `Bundle.module` accessor → `cannot find
  'PackageResources' in scope`,
- spawns a `ClaudeSpyPackage_GallagerEmoji.bundle` resource-bundle target,
- builds the library as a **dynamic framework** (`PackageFrameworks/`) —
  which would break the bare single-file `GallagerCLI` in
  `Gallager.app/Contents/MacOS/` at runtime even if the compile were fixed.

Single-client graphs behave: the iOS app, `swift build`/`swift test`, and
`xcodebuild` against the package's own schemes all honor `.embedInCode` —
so local package tests pass while the app build breaks. Upstream context:
...
Alternatives if this ever needs revisiting: a build-tool plugin emitting
the same string literal, or restructuring how the app forces the CLI build
so `GallagerEmoji` has a single consumer per graph.

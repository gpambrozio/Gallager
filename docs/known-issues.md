...
## Xcode dynamicizes package targets shared between the app and GallagerCLI

**Status:** Root behavior is an Xcode limitation; the CLI consequence is
fixed by an rpath in the copy phase. Documented so neither the "obvious
cleanup" (`.embedInCode`) nor the "obvious simplification" (removing the
rpath line) is attempted again.

**The behavior (Xcode 26.6, verified with clean Debug and Release builds
plus build-log inspection):** when one local package target is consumed by
**two products in the same build graph** — `GallagerEmoji` feeds both the
app's library stack *and* the `GallagerCLI` executable product (which the
app target links to force its build for the copy-into-bundle phase) —
XCBuild stops linking that target statically and silently builds it **once
as a dynamic framework** (`PackageFrameworks/GallagerEmoji.framework`).
This happens **with or without resources**, and removing the (unused)
`GallagerEmoji` library product does not prevent it. Single-client graphs
are unaffected: the iOS app, `swift build`/`swift test`, and `xcodebuild`
against the package's own schemes all link statically.

**Consequence 1 — the CLI dyld crash (fixed).** The "Copy GallagerCLI to
bundle" phase copies the bare binary into `Contents/MacOS/`, which then
needs `@rpath/GallagerEmoji.framework` at launch. Debug binaries carry an
absolute DerivedData rpath (so dev-machine runs and e2e pass by accident);
Release binaries carry only `/usr/lib/swift`, so **every** `gallager`
invocation in a shipped app died in dyld before `main()`. Fix: the copy
phase runs `install_name_tool -add_rpath "@loader_path/../Frameworks"`
before codesigning, pointing the CLI at the framework Xcode already embeds
...
data (instead of the generated `EmojiData.swift` string literal) was
designed, implemented, and reverted on PR #632 (spec:
`docs/superpowers/specs/2026-07-03-emoji-data-shipping-design.md`). On the
dynamic-framework path above, XCBuild also reverts resource handling to a
`Bundle.module` accessor and a `ClaudeSpyPackage_GallagerEmoji.bundle`
target, so the `.embedInCode`-generated `PackageResources` symbol never
exists → `cannot find 'PackageResources' in scope`, macOS app build only.
Upstream context:
https://github.com/apple/swift-package-manager/issues/6969

The emoji table therefore stays baked into Swift source
(`EmojiData.swift`, regenerated by `scripts/generate-emoji-data.py`) —
also the measured optimum: the ~12ms lazy parse is dominated by search
tokenization (~7ms) that no format change removes, a codegen'd
`[Emoji(...)]` literal costs ~96s of compile time, and a binary-plist
resource decodes *slower* than the tab-split loop. If the shared-target
dynamicization ever needs a real fix: build the CLI out-of-graph (copy
phase runs `swift build --product GallagerCLI`, always static).

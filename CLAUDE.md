...
- **pi sidecar plugin (real example):** `plugins/pi/` - a working sidecar adding [pi](https://github.com/earendil-works/pi) (the `pi` coding-assistant CLI, `@earendil-works/pi-coding-agent`) support: Python `bin/sidecar` + a pi *extension* bridge (`pi-extension/gallager.ts`, installed to `~/.pi/agent/extensions/gallager.ts`) — pi's integration surface is its rich extension **event bus** (`pi.on("session_start"|"agent_start"|…)`), not shell hooks. pi's events are explicit, so unlike opencode the mapping needs **no state machine**: `session_start`→`idle` (session appears — nothing else surfaces it before the first turn, mirroring Claude's SessionStart), `agent_start`→`working`, `agent_end`→`doneWorking` (attention badge + a notification; summary = the turn's last assistant text line, extracted in the extension), `session_shutdown` with `reason:"quit"`→`AppAction.sessionEnded` keyed by the PANE id (reload/new/resume/fork are ignored — pi tears down + re-announces with a fresh `session_start`, so ending there would flicker the row). pi's quit key is **Ctrl-D** (`app.exit`, editor empty), not `/exit` or Ctrl-C (those clear/cancel). **Telemetry (#617):** the extension POSTs one OTLP/JSON record per completed assistant `message_end` (`AssistantMessage.usage`) to the receiver's `/v1/logs`, event name `pi.api_request`, Claude's exact attribute keys (pi's `reasoning` tokens folded into `output_tokens`), `session.id` = **pi's session UUID** (`ctx.sessionManager.getSessionId()`) — the same id the sidecar stamps on its `PluginEvent`s, so the host's telemetry join key matches (`duration_ms` = the extension's message_start→message_end wall time; dedup by `responseId`). The sidecar bakes the ingress socket path + plugin id + OTLP endpoint into the extension at install via `__GALLAGER_INGRESS_SOCK__`/`__GALLAGER_PLUGIN_ID__`/`__GALLAGER_OTLP_ENDPOINT__` token substitution (pi doesn't inherit Gallager's env; `GALLAGER_*` env fallbacks serve repo smoke tests; stale if the receiver's port changes → re-Install). Projects surface in the sidebar "+" menu by reading the real `cwd` from the newest session file's `{"type":"session",…,"cwd":…}` header per dir under `~/.pi/agent/sessions/<slug>/*.jsonl` (the dir slug is a lossy path slugification — the header isn't; skips cwds that no longer exist). Gotcha: `PluginEvent.appActions` is non-optional — every event JSON MUST include `"appActions": []` or the host silently drops it. Scope: working/idle/attention + tokens + lifecycle + projects; pi's tool-approval prompts are **not** wired to interactive forms yet (`awaitingPermission`/`deliver_response` is a follow-on). Tests: `python3 plugins/pi/tests/test_sidecar.py` (23). `./scripts/dev-install.sh` **copies** it into `~/.gallager/plugins/`.

...
- **Update hosting (maintainers, #664):** Sparkle appcast (`ClaudeSpy.xml`) + DMGs + plugin bundles are served from `/opt/claudespy-updates` on the relay box behind `updates.gallager.app` (Caddy `file_server`, `ClaudeSpyPackage/caddy/updates.caddy`, installed by `deploy.sh` on every prod deploy). `release.sh` uploads over rsync/SSH (host: `DEPLOY_HOST` > `hcloud` `cleancast`; no more FTP/lftp/1Password). The legacy `updates.gustavo.eng.br` hostname is baked into every shipped `SUFeedURL` and must keep serving forever — it becomes an alias in `updates.caddy` (currently commented out; uncomment only after its DNS points at the box, or ACME retries noisily). `docs/ClaudeSpy.xml` keeps only the last 2 versions.

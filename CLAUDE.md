...
- **E2E testing:** `docs/e2e-testing.md` - Test framework, running tests, writing scenarios, video recording (`--record`, issue #621), shell-history isolation (every e2e shell runs under a `$ZDOTDIR` shim so typed commands never reach `~/.zsh_history`)

...
@Suite("ClaudeCodeKeystrokeBuilder")
struct ClaudeCodeKeystrokeBuilderTests {
    // MARK: - Fixtures
    ...
    func singleSelectFirstOption() {
        ...
        let steps = ClaudeCodeKeystrokeBuilder().keystrokes(
            for: response,
            matching: request
        )
    ...
    func multiSelectTwoOptions() {
        ...
        // Multi-select: Enter toggles in place. From 0 → toggle, then
        // 2 steps down (to index 2), toggle. Then a trailing commit
        // Enter since the question is multi-select.
    ...
    func multiSelectOther() {
        ...
        // From 0: enter (toggle), then 2 steps down to "Other", text,
        // space + down + enter (toggle Other + advance past it), then
        // trailing commit enter for multi-select.
        #expect(steps == [
    ...
    func twoSingleSelectQuestions() {
        ...
        // Per-question enter to select + advance, then trailing enter
        // since the batch has more than one question.
    ...
    func textOnlyNonEmpty() {
        let steps = ClaudeCodeKeystrokeBuilder().keystrokes(forText: "hi")
        #expect(steps == [.text("hi"), .keys([.enter])])
    }

    @Test("Text-only response with empty text → just Enter (interrupt)")
    func textOnlyEmpty() {
        let steps = ClaudeCodeKeystrokeBuilder().keystrokes(forText: "")
        #expect(steps == [.keys([.enter])])
    }
}

    ...
    // MARK: - CodexSidecarTests

    /// Integration tests for the `CodexPluginSidecar` executable.
    /// Each test spawns the SPM-built binary as a child process and drives
    /// it via JSON-RPC over Pipes. The ingress socket itself is covered by
    /// `IngressSocketServerTests`; we deliberately avoid driving it from a
    /// sibling process here because `NWListener`'s Unix socket files aren't
    /// always visible across the test bundle's sandbox (the child process
    /// binds successfully but the test runner can't see the path).
    ///
    ...
    /// parallel.
    @Suite("CodexPluginSidecar", .serialized)
    struct CodexSidecarTests {
        // MARK: - Binary resolution

        /// Resolve the SPM-built sidecar binary. Walks `.build/<config>/`
        /// looking for the first one that has `CodexPluginSidecar` —
        /// matches the approach `ClaudeCodeSidecarTests` uses for its
        /// fixture.
        private func sidecarURL() -> URL? {
            let buildDir = URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent() // CodexPluginSidecarTests
            ...
            for config in configs {
                let candidate = buildDir
                    ...
                    .appendingPathComponent("CodexPluginSidecar")
        ...
        // the bundled `CodexEventTranslator` to confirm a
        ...
        func sidecarTranslateEventReturnsPluginEvent() async throws {
            guard let binary = sidecarURL() else {
                Issue.record("CodexPluginSidecar binary missing — run `swift build` first")
                return
            }
            ...
            let pluginRoot = try makeTempDir(label: "codex-plugin-root")
            ...
            let stateDir = try makeTempDir(label: "codex-plugin-state")
            defer {
            ...
            // 3. Send a `translate_event` RPC. The Codex translator
            // maps a `UserPromptSubmit` payload onto `working: true`
            // and emits `dismissFileSuggestions` as an AppAction.
            let translateRequest = JSONRPCRequest(
                ...
                params: .object([
                    "context": .object([
                        "TMUX_PANE": .string("%0"),
                        "CODEX_PROJECT_DIR": .string("/tmp/proj"),
                        "CODEX_SESSION_ID": .string("session-1"),
                    ]),
                    "payload": .object([

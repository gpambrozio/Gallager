    ...
    /// Top-level Codex sidecar orchestrator (Spec §6).
    ///
    /// Symmetric to `ClaudeCodeSidecar`: owns the JSON-RPC loop bound to
    /// stdin/stdout, the ingress socket server for hook bridge scripts,
    /// the FSEvents-backed project watcher (rooted at `~/.codex/sessions/`
    /// instead of `~/.claude/projects/`), the event translator, the
    /// keystroke builder, and the per-session request store. `run()` exits
    /// cleanly on `shutdown` RPC or stdin EOF.
    ///
    ...
    @MainActor
    final class CodexSidecar {
        // MARK: - State

        private let logger: Logger
        private let dispatcher: RPCDispatcher
        private let translator: CodexEventTranslator
        private let keystrokeBuilder: CodexKeystrokeBuilder
        ...
        init(
            ...
            let logger = logger ?? Logger(label: "codex.sidecar")
            self.logger = logger
            self.stdin = stdin
            self.stdout = stdout
            self.writeQueue = WriteQueue(handle: stdout, logger: logger)
            self.dispatcher = RPCDispatcher(logger: logger)
            self.translator = CodexEventTranslator(logger: logger)
            self.keystrokeBuilder = CodexKeystrokeBuilder()
        ...
        private func handleInitialize(params: JSONValue?) async throws -> JSONValue {
            ...
            // bundled Codex plugin doesn't add any beyond the settings
        ...
        private func handleInstall(params: JSONValue?) async throws -> JSONValue {
            ...
            let codexCommand = try await resolveCodexCommand(override: decoded?.codexCommand)
            @Dependency(CodexInstaller.self) var installer
            do {
                try await installer.install(codexCommand)
        ...
        private func handleUninstall(params: JSONValue?) async throws -> JSONValue {
            ...
            @Dependency(CodexInstaller.self) var installer
            do {
                try await installer.uninstall(codexCommand)
                return .object(["status": .string("ok")])
            } catch {
                return .object([
                    "status": .string("failed"),
        ...
        private func handleIsInstalled(params: JSONValue?) async throws -> JSONValue {
            ...
            let codexCommand = try await resolveCodexCommand(override: decoded?.codexCommand)
            @Dependency(CodexInstaller.self) var installer
            let installed = await installer.isInstalled(codexCommand)
        ...
        private func handleApplySettings(params: JSONValue?) async throws -> JSONValue {
            ...
            do {
                settings = try CodexSettings.decode(from: decoded.settings)
            } catch {
                return .object([
                    "status": .string("error"),
        ...
        private func startProjectWatcher() async {
            ...
            let watcher = FSEventsProjectWatcher(
                paths: watchPaths,
                dispatchQueueLabel: "gallager.plugin.codex.fsevents",
                logger: logger
            )
        ...
        /// (`CODEX_HOME/sessions` when the env override is set). We watch
        /// that root; if the directory doesn't exist yet, the watcher
        /// silently no-ops and the sidecar relies on manual
        /// `refresh_projects` calls.
        ...
        private func scanAndPushProjects() async {
            @Dependency(CodexProjectScanner.self) var scanner
        ...
        private func loadSettings() -> CodexSettings {
            guard
                let stateDir,
                let data = try? Data(contentsOf: stateDir.appendingPathComponent("settings.json")),
                let settings = try? JSONDecoder().decode(CodexSettings.self, from: data)
            else {
        ...
        private func resolveCodexCommand(override: String?) async throws -> String {
            if let override, !override.isEmpty {
                return override
            }
            // Pull from saved settings — falls back to the default `codex`
            // command name if no settings are on disk yet.
            let settings = loadSettings()
            let candidate = settings.commandPath.trimmingCharacters(in: .whitespacesAndNewlines)
            return candidate.isEmpty ? "codex" : candidate
        }

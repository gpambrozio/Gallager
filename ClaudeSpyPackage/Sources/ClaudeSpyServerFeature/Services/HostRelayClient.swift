...
public enum HostRelayClientError: Error, LocalizedError {
    case notConnected
    case timeout
...
/// Client for connecting to the external relay server via WebSocket as a Mac viewer.
///
/// Handles bidirectional communication between a Mac viewer and the relay server,
/// receiving session state and terminal streams from a Mac host.
@Observable
@MainActor
final public class HostRelayClient {
    ...
    public enum ConnectionState: Equatable, Sendable {
        ...
        public var statusText: String {
            ...
            case .extendedBackoff: "Extended backoff..."
    ...
    private let logger = Logger(label: "com.claudespy.hostrelayclient")

    /// Current connection state
    public private(set) var state: ConnectionState = .disconnected

    /// Whether the Mac host is currently connected to the relay
    public private(set) var isMacConnected = false

    /// Name of the connected Mac host device (if known)
    ...
    /// Called when a hook event is received from Mac host
    public var onHookEvent: (@Sendable (HookEventMessage) -> Void)?

    /// Called when session state is received from Mac host
    public var onSessionState: (@Sendable (SessionStateMessage) -> Void)?

    /// Called when a terminal stream message is received from Mac host
    ...
    /// Connect to the external relay server as a Mac viewer
    public func connect(
        serverURL: URL,
        pairId: String,
        deviceId: String,
            ...
            pendingCommands[commandMessage.id] = { result in
                ...
                case let .success(anyResponse):
                    if let typedResponse = anyResponse as? C.Response {
                        ...
                        continuation.resume(returning: .failure(HostRelayClientError.commandFailed("Unexpected response type")))
                    }
    ...
    /// Request current session state from Mac host
    ...
    private func performConnect() async {
        ...
        // Connect as macViewer device type
        components?.queryItems = [
            URLQueryItem(name: "pairId", value: pairId),
            URLQueryItem(name: "deviceType", value: "macViewer"),
        ...
        logger.info("Connecting to relay server as Mac viewer: \(wsURL)")
        ...
        // Send registration message as Mac viewer
        let registerMessage = WebSocketMessage.registerMacViewer(
            RegisterMacViewerMessage(
                pairId: pairId,
                deviceId: deviceId,
    ...
    private func handleWebSocketMessage(_ message: WebSocketMessage) async {
        ...
        case let .macViewerRegistered(response):
            if response.success {
                logger.info("Successfully registered with relay server as Mac viewer")
                state = .connected
                connectedMacName = response.macDeviceName
                isMacConnected = response.macDeviceName != nil

                // Establish E2EE session if Mac host is connected and we have their public key
                if
                    ...
                    do {
                        ...
                        logger.info("E2EE session established with Mac host")
                ...
                // Request session state if Mac host is connected
                if isMacConnected {
        ...
        case let .hookEvent(event):
            logger.info("Received hook event from Mac host")
            onHookEvent?(event)

        case let .sessionState(sessionState):
            logger.info("Received session state from Mac host")
            onSessionState?(sessionState)

        case let .commandResponse(response):
            logger.info("Received command response from Mac host")
            if let handler = pendingCommands.removeValue(forKey: response.commandId) {
                if response.success {
                    handler(.success(response))
                } else {
                    handler(.failure(HostRelayClientError.commandFailed(response.error ?? "Unknown error")))
                }
            }
        ...
        case let .macConnected(connectedMessage):
            logger.info("Mac host connected")
            isMacConnected = true
            ...
            if
                ...
                do {
                    ...
                    partnerPublicKey = macPublicKey
                    partnerPublicKeyId = macPublicKeyId
                    logger.info("E2EE session established with Mac host on connect notification")
        ...
        case .macDisconnected:
            logger.info("Mac host disconnected")
        ...
        default:
            logger.debug("Received unhandled message type: \(decryptedMessage.messageType)")
        }
    }

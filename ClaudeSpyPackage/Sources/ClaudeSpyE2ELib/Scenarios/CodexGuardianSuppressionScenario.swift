...
/// 1. An auto-approvable `PermissionRequest` (`permission_mode: "default"`)
///    produces NO iOS response form, NO push, and the session stays Working.
...
/// 4. Rewriting `config.toml` flips the behavior live in BOTH directions via
///    the FSEvents config watcher — no new SessionStart needed (the Codex TUI
///    persists every "Approve for me" toggle to this file).
///
...
public enum CodexGuardianSuppressionScenario {
    public static let scenario = ClaudeSpyE2ELib.scenario(
        ...
        //          prompt follows: must still notify and form.
        // ══════════════════════════════════════════════════════════════
        TestStep.macSendHookEvent(
            pluginID: "codex",
            json: """
        ...
        // The form's file-path description is unique on screen (the phase-4
        // "All questions answered" feedback is still visible until replaced).
        TestStep.iosWaitForElement(.labelContains("/tmp/data.txt"), timeout: 10)
        ...
        TestStep.assertStoredContains(
            ...
            substring: "Permission: Read|${codexGuardianPane}"
        )
        ...
        //          TUI does when toggling "Approve for me" off). The
        //          FSEvents config watcher picks it up live — the same
        //          request now notifies and forms. No new SessionStart.
        // ══════════════════════════════════════════════════════════════
        TestStep.writeFile(
            path: "${gallagerStateRoot}/codex-home/config.toml",
            content: "approvals_reviewer = \"user\"\n"
        )
        // FSEvents latency (~0.5s) + watcher debounce (0.5s) + margin.
        ...
        // The Write form's file-path description proves the NEW form arrived
        // (the lingering "Permission accepted" feedback from Phase 5 would
        // also match a bare "Accept" contains-query).
        TestStep.iosWaitForElement(.labelContains("/tmp/out.txt"), timeout: 10)
        ...
        TestStep.assertStoredContains(
            ...
            substring: "Permission: Write|${codexGuardianPane}"
        )
        TestStep.iosTap(.label("Accept"))
        TestStep.iosWaitForElement(.labelContains("Permission accepted"), timeout: 5)

        // ══════════════════════════════════════════════════════════════
        // Phase 7: Flip back to auto_review — suppression resumes, again
        //          with no new SessionStart.
        // ══════════════════════════════════════════════════════════════
        TestStep.writeFile(
            path: "${gallagerStateRoot}/codex-home/config.toml",
            content: "approvals_reviewer = \"auto_review\"\n"
        ...
        // The Glob form's pattern never appears on iOS, and no push went out.
        TestStep.iosWaitForElementToDisappear(.labelContains("*.swift"), timeout: 5)
        TestStep.readFile(path: "${pushLogPath}", storeAs: "pushLogAfterGuardianGlob")
        TestStep.assertStoredNotContains(
            key: "pushLogAfterGuardianGlob",
            substring: "Permission: Glob|${codexGuardianPane}"
        )
    }
}

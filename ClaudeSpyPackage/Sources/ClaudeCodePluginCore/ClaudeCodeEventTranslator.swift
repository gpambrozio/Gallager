...
// MARK: - ClaudeCodeRequestStore

/// Tracks outstanding `AgentResponseRequest`s emitted by the translator so the
/// keystroke builder can later match a `deliver_response` back to the original
/// question shape (per Spec §7.5.1: "the sidecar may want to keep some state
/// per outstanding request_id — e.g., the original question text so it can
/// decide keystroke navigation").
///
...
public actor ClaudeCodeRequestStore {
...
/// `ClaudeCodeRequestStore` is an actor (every response-request emission
...
public struct ClaudeCodeEventTranslator: Sendable {
    ...
    public func translate(
        ...
        requestStore: ClaudeCodeRequestStore

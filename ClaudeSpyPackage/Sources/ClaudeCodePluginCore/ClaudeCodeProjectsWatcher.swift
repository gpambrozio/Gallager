    ...
    /// Watches `~/.claude/projects/` for changes and fires a debounced callback so
    /// the plugin core can rescan projects when Claude writes a new transcript.
    ///
    ...
    /// transcript writes triggers a single rescan. macOS-only — on Linux the core
    /// falls back to the initial scan + manual refresh.
    ///
    /// Not an actor: `FSEventStream` requires a callback with a C function
    /// pointer, so the bridging context and debounce timer are guarded by an
    /// internal lock instead. The public surface is `Sendable`.
    final class ClaudeCodeProjectsWatcher: @unchecked Sendable {
        private let path: String
        private let debounce: DispatchTimeInterval
        private let onChange: @Sendable () -> Void

        private let queue = DispatchQueue(label: "com.gallager.claude-code.projects-watcher")
        private var stream: FSEventStreamRef?
        private var debounceWorkItem: DispatchWorkItem?

        /// - Parameters:
        ///   - path: Directory to watch (e.g. `~/.claude/projects`).
        ...
        private static let callback: FSEventStreamCallback = { _, info, _, _, _, _ in
            ...
            let watcher = Unmanaged<ClaudeCodeProjectsWatcher>.fromOpaque(info).takeUnretainedValue()
